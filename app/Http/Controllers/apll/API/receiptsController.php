    ...
    public function store(Request $request){
        ...
        try{
            ...
            if ($operation->id == 4){
                ...
                $tree = supplier::where('id',$value['tree'])->first()['tree_id'];
            }elseif ($operation->id == 2){
                $tree = subscription::where('id',$value['tree'])->first()['tree_id'];
            }elseif ($operation->id == 3){
                $tree = Project::where('id',$value['tree'])->first()['tree_id'];
            }elseif ($operation->id == 5){
                $tree = employee::where('id',$value['tree'])->first()['tree_id'];
            }elseif ($operation->id == 10){
                $tree = Contractors::where('id',$value['tree'])->first()['tree_id'];
            }
    ...
    public function rgetcc($id,$operations){
        if ($id && $operations){
            ...
            if ($operations == 4){
                ...
                if (subscription::where('id',$id)->first()->cc_type != 0){
                    $cc = 1;
                    $glcc = glcc::where('type','1')->pluck('name_ar','id');
                }else{
                    $cc = 0;
                ...
                if (Project::where('id',$id)->first()->cc_id != null){
                    $cc_id = Project::where('id',$id)->first()->cc_id;
                    $cc = 1;
                    $glcc = glcc::where('type','1')->where('parent_id',$cc_id)->pluck('name_ar','id');
                }else{
                    $cc = 0;
                }
            }elseif ($operations == 5){
...
//            }
            ...
            foreach (array_slice($requests, 0, count($requests) - 1) as $key => $r){
                ...
                if ($operation->id == 4){
                    ...
                    $tree = supplier::where('id',$r['relation_id'])->first()['tree_id'];
                }elseif ($operation->id == 2){
                    $tree = subscription::where('id',$r['relation_id'])->first()['tree_id'];
                }elseif ($operation->id == 3){
                    $tree = Project::where('id',$r['relation_id'])->first()['tree_id'];
                }elseif ($operation->id == 5){
                    $tree = employee::where('id',$r['relation_id'])->first()['tree_id'];
                }elseif ($operation->id == 10){
                    $tree = Contractors::where('id',$r['relation_id'])->first()['tree_id'];
                }
