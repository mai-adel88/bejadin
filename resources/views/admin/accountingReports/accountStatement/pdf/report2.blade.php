...
@foreach ($data as $day => $receipts)
    ...
    <div class="el-account">
        @foreach($receipts as $key => $receipt) @if($key == 0) <p>رقم الحساب : {{$receipt->departments['code']}}</p> @endif  @endforeach
        <p>اسم الحساب : {{$day}}</p>
    </div>
    <div class="el-date">
        <p>من تاريخ : {{$from}}</p>
            ...
            @foreach($receipts as $key => $receipt)
                @if($key == 0)
                    <tr>
                        ...
                        <td>
                            @if($key  == 0)
                                {{firstdatelaccount($from,$receipt->tree_id,$receipt->operation_id)->sum('debtor') + firstdateraccount($from,$receipt->tree_id,$receipt->operation_id)->sum('debtor') + \App\receiptsData::where('tree_id',$receipt->tree_id)->whereHas('receipts',function ($query) use ($from) {
                                return $query->where('created_at','<',$from);})->sum('debtor')}}
                                <div class="hidden">{{$dataDebtor += firstdatelaccount($from,$receipt->tree_id,$receipt->operation_id)->sum('debtor') + firstdateraccount($from,$receipt->tree_id,$receipt->operation_id)->sum('debtor') + \App\receiptsData::where('tree_id',$receipt->tree_id)->whereHas('receipts',function ($query) use ($from) {
                                return $query->where('created_at','<',$from);})->sum('debtor')}}</div>
                            @endif
                        </td>
                        <td class="creditor">
                            @if($key  == 0)
                                {{firstdatelaccount($from,$receipt->tree_id,$receipt->operation_id)->sum('creditor') + firstdateraccount($from,$receipt->tree_id,$receipt->operation_id)->sum('creditor') + \App\receiptsData::where('tree_id',$receipt->tree_id)->whereHas('receipts',function ($query) use ($from) {
                                ...
                                return $query->where('created_at','<',$from);})->sum('creditor')}}</div>
                            @endif
                        </td>
                        <td>
                            @if($key  == 0)
