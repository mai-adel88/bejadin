...
<div class="table-responsive">
    <table style="border: none" class="table table-bordered table-striped table-hover text-center">
        ...
        @foreach($departments as $merged)
            @if(departmentsum($merged->id,$from,$to,'creditor','>=') == 0 && departmentsum($merged->id,$from,$to,'debtor','>=') == 0 && alldepartmenttrial($merged->id,$from,$to,'debtor','>=') == 0 && alldepartmenttrial($merged->id,$from,$to,'creditor','>=') == 0)
                <tr>
                    <td>
                        {{$merged->code}}
                    </td>
