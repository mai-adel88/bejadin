...
<div class="table-responsive">
    <table style="border: none" class="table table-bordered table-striped table-hover text-center">
        ...
        @foreach($departments as $merged)
            <div style="display: none;">
                {{$debtor = sumall2($merged->id,null,$from,$to,'debtor','<')}}
                {{$creditor = sumall2($merged->id,null,$from,$to,'creditor','<')}}
                {{$debtor1 = sumall2($merged->id,null,$from,$to,'debtor','<')}}
                {{$creditor1 = sumall2($merged->id,null,$from,$to,'creditor','<')}}
                {{$creditor2 = sumall($merged->id,null,$from,$to,'creditor','>=')}}
                {{$debtor2 = sumall($merged->id,null,$from,$to,'debtor','>=')}}
            ...
            <tr>
                ...
                <td>
                    {{$creditor2 = sumall($merged->id,null,$from,$to,'creditor','>=')}}
                    <div style="display:none">
                        {{$dataCredit1 += sumall($merged->id,null,$from,$to,'creditor','>=')}}
                    </div>
                </td>
                <td>
