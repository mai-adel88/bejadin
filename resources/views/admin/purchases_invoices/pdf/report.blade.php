...
<div style="display: none">
    ...
    <div class="el-no3">
        ...
        <p>رقم الفاتورة : {{$invoices[0]->header->Doc_No }} </p>
    ...
    <div class="el-date">
        ...
        <p>الشركة : {{$invoices[0]->header->company->{ 'Cmp_Nm' .ucfirst(session('lang')) } }} </p>
        <p>الفرع : {{$invoices[0]->header->branch->{ 'Brn_Nm' .ucfirst(session('lang')) } }}</p>
        <p>العميل : {{$invoices[0]->header->customer->{ 'Cstm_Nm' .ucfirst(session('lang')) } }}</p>
        <p>المستودع : {{$invoices[0]->header->store->{ 'Dlv_Nm' .ucfirst(session('lang')) } }}</p>
        <p>مندوب المبيعات : {{
        App\Models\Admin\AstSalesman::
       where('Cmp_No',$invoices[0]->header->Cmp_No)

            ->where('Slm_No',$invoices[0]->header->Slm_No)->first()->Slm_NmAr
    ...
    <div class="el-data">
        <p>تاريخ الفاتورة : {{$invoices[0]->header->Doc_Dt}}</p>
        <p>تاريخ سداد الفاتورة : {{$invoices[0]->header->Pym_Dt}}</p>
    ...
    <div class="table-responsive" style="page-break-after:auto;">
        <table style="border:none" class="table table-bordered table-striped table-hover" data-serial="{{$s++}}">
            ...
            @foreach($invoices as $merged)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <div style="display: none">
                            {{$total +=($merged->Qty * $merged->Itm_Sal) - $merged->Disc1_Val }}
                            </div>
                        </td>
            ...
            <tr>
                ...
                <th> {{$invoices[0]->header->Net}}</th>
                <th >بعد الخصم <span style="padding-right:15px"> </span></th>
                <th> {{$total}}</th>
                <th >قيمة الضريبة <span style="padding-right:15px"> </span></th>
                <th> {{$invoices[0]->header->Taxv_Extra}}</th>
