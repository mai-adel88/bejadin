...
<div class="clearfix"></div>
        ...
        <div class="invoice-info">
            ...
            <div class="invoice-col">
                ...
                <span id="block_chain">
                    @foreach($data as $d)
                        ...
                        @if($d->cc_id && $d->cc_id != null)
                            / <span id="block_chain">{{$d->glcc->name_ar}}</span>
                        @endif
                    @endforeach
                </span>
            </div>
            <div class="invoice-col">
                {{trans('admin.paid_is')}} :
                <span id="block_chain">
                    <strong>{{trans('admin.just')}} {{makeNumber2Text($receiptsData->debtor)}} {{trans('admin.EGP')}} {{trans('admin.not_else')}}</strong>
                </span>
            </div>
            <div class="invoice-col">
        ...
        <table>
            <tr>
                ...
                <td style="padding-right: 5px"> {{makeNumber2Text($receiptsData->debtor)}} {{trans('admin.EGP')}}</td>
            </tr>
            <tr>
                <th>{{trans('admin.rest')}} : </th>
                <td style="padding-right: 5px"> {{makeNumber2Text($data->sum('creditor') - $receiptsData->debtor)}} {{trans('admin.EGP')}}</td>
            </tr>
        </table>
        ...
        <div style="border: 2px solid #000;padding:10px">
            <table>
                <tr>
                    ...
                    <td style="text-align: right">{{ session_lang(\App\Branches::where('id',$receipts->branche_id)->first()->name_en,\App\Branches::where('id',$receipts->branche_id)->first()->name_ar) }}</td>
                </tr>
                <tr>
                    <th style="text-align: right">{{trans('admin.sUrl')}} :</th>
                    <td style="text-align: right">{{\App\Branches::where('id',$receipts->branche_id)->first()->addriss}}</td>
                </tr>
                <tr>
