...
<script>
    $(function () {
        ...
        $(".type").on("change",function(){
            ...
            $.ajax({
                    url: '{{aurl('openingentrydata/select')}}',
                    type:'post',
                    dataType:'html',
    ...
    $(function () {
        ...
        $('#add_data').on('click', function() {
            ...
            $.ajax({
                ...
                type:'post',
                dataType:'html',
                data:{_token: "{{ csrf_token() }}",type: type,add : add,invoice:invoice,note_debtor: note_debtor,tree: tree,limitations: limitations,dbt: dbt,crd: crd,month_for: month_for},
                success: function (data) {
...
<div class="box">
    ...
    <div class="box-body">
        <div class="row text-center">
            ...
            <div class="col-md-3">
                <strong>{{trans('admin.limitation')}} </strong>: {{session_lang($limitationReceipts->name_en,$limitationReceipts->name_ar)}}
            </div>
        </div>
        <br>
        <opening-editcomponent :invoice="{{$limitations->invoice_id}}" :id="{{$limitations->id}}" :branchesid="{{$limitations->branche_id}}" created_at="{!! date('Y-m-d',strtotime($limitations->created_at)) !!}" date="{{date('Y-m-d',strtotime($limitations->date))}}" :limitationsnumber="{{$limitations->limitationId}}"></opening-editcomponent>
    </div>
</div>
@endsection
