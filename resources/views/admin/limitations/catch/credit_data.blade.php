<div class="col-md-6">
    <div class="panel panel-primary">
        ...
        <input type="text" name="getSalNo" id="getSalNo" value="{{$trans->Slm_No}}" hidden>
        <input type="text" name="getSalName" id="getSalName"
            value="{{\App\Models\Admin\AstSalesman::where('Slm_No', $trans->Slm_No)->pluck('Slm_Nm'.ucfirst(session('lang')))->first()}}" hidden>
        <div class="panel-body">
            <input type="text" name="Ln_No" id="Ln_No" value="{{$trans->Ln_No}}" hidden>
            {{-- الحساب الرئيسى --}}
            <div class="row">
                <div class="col-md-8">
                    ...
                    value="{{\App\Models\Admin\MtsChartAc::where('Acc_No', $trans->Acc_No)->pluck('Acc_Nm'.ucfirst(session('lang')))->first()}}">
                </div>
                <div class="col-md-4">
                    <label for="Acc_No"></label>
                    <input type="text" name="Acc_No" id="Acc_No" class="form-control" disabled value="{{$trans->Acc_No}}">
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <select name="Ac_Ty" id="Ac_Ty" class="form-control">
                        ...
                        @foreach(\App\Enums\AccountType::toSelectArray() as $key => $value)
                            <option value="{{$key}}" @if($trans->Ac_Ty == $key) selected @endif>{{$value}}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <select name="Acc_No_Select" id="Acc_No_Select" class="form-control select2">
                        ...
                        @if($subAccs == null)
                            @foreach($subAccs as $sub)
                                <option value="{{$sub->no}}" @if($sub->no == $trans->Sysub_Account) selected @endif>{{$sub->name}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="Sysub_Account" id="Sysub_Account" class="form-control" value="{{$trans->Sysub_Account}}">
                </div>
            </div>
            {{-- نهاية نوع الحساب --}}
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="Tr_Cr" id="Tr_Cr" class="form-control" value="{{$trans->Tr_Cr}}">
                </div>
                {{-- نهاية المبلغ دائن --}}
                {{-- رقم المستند --}}
                <div class="col-md-4">
                    <label for="Dc_No">{{trans('admin.receipt_number')}}</label>
                    <input type="text" name="Dc_No" id="Dc_No" class="form-control" value="{{$trans->Dc_No}}">
                </div>
                {{-- نهاية رقم المستند --}}
                {{-- مركز التكلفه --}}
                    ...
                    <select name="Costcntr_No" id="Costcntr_No" class="form-control select2">
                        @if(count($cost_center) > 0)
                            @foreach($cost_center as $cc)
                                <option value="{{$cc->Costcntr_No}}" @if($trans->Costcntr_No == $cc->Costcntr_No) selected @endif>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="Tr_Ds" id="Tr_Ds" class="form-control col-md-6" value="{{$trans->Tr_Ds}}">
                </div>
                {{-- نهاية البيان عربى --}}
                {{-- البيان انجليزى --}}
                <div class="col-md-12">
                    ...
                    <input type="text" name="Tr_Ds1" id="Tr_Ds1" class="form-control col-md-6" value="{{$trans->Tr_Ds1}}">
