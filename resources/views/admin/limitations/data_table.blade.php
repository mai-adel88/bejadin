...
@foreach($data as $d)
    <tr>
        ...
        <td><a class="btn btn-danger waves-effect waves-light remove-record" data-toggle="modal" data-id="{{$d->id}}"><i class="fa fa-trash"></i> {{trans('admin.delete')}}</a></td>
    </tr>
@endforeach
    <tr>
        <td colspan="2"><strong>{{trans('admin.total_motion_creditor')}}</strong></td>
        <td colspan="5"><strong>{{$data->sum('creditor')}} {{trans('admin.EGP')}}</strong></td>
    ...
    <tr>
        ...
        <td colspan="6"><strong>{{$data->sum('debtor')}} {{trans('admin.EGP')}}</strong></td>
