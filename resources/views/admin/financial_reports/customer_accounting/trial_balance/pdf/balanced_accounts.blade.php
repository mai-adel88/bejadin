...
<div class="table-responsive">
    <table style="border: none" class="table table-bordered table-striped table-hover text-center">
        ...
        @foreach($data as $merged)
            @if ( ($merged->Fbal_DB +
                FbalanceCust($merged->Cmp_No,$merged->Sysub_Account,$From,$to,'Tr_Db','<')) > 0 ||
                ($merged->Fbal_CR  +
                 FbalanceCust($merged->Cmp_No,$merged->Sysub_Account,$From,$to,'Tr_Cr','<')) > 0 ||
                 (getTransCust($merged->Cmp_No,$merged->Acc_No,$From,$to,'Tr_Db','>=')) > 0 ||
                 (getTransCust($merged->Cmp_No,$merged->Acc_No,$From,$to,'Tr_Cr','>=')) > 0
                )
                <tr>
                    <td>
                        {{$merged->Acc_No}}
