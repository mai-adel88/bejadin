...
<div class="table-responsive">
    <table style="border: none" class="table table-bordered table-striped table-hover text-center">
        ...
        @foreach($data as $merged)
            @if (
                ($merged->Fbal_DB  +
                FbalanceSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Db','<'))> 0||
                ($merged->Fbal_CR  +
                FbalanceSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Cr','<'))> 0||
                (getTransSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Db','>='))>
                0|| (getTransSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Cr','>=')) > 0
                )
                <tr>
                    <td>
                    ...
                    <td class="depratment_move_debter">
                        ...
                        {{$value_debtor2 = getTransSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Db','>=')}}
                        <div style="display:none">
                            {{$dataDebtor1 += getTransSup($merged->Cmp_No,$merged->Acc_No,$from,$to,'Tr_Db','>=')}}
                        </div>
                    </td>
